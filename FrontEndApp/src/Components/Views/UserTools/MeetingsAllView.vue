...
const API_BASE = 'http://localhost:8080/api/meets/all-meets'
...
async function fetchMeetings() {
  ...
  try {
    ...
    error.value = 'Failed to load meetings. ' + String(e?.message ?? e)
  } finally {
    loading.value = false
  }
}
...
watch(filteredMeetings, async (newList) => {
  ...
  if (dayCell) {
    ...
    dayCell.classList.add('highlight')   // анімаційне підсвічування
  ...
  <div class="page">
    <header class="header">
      <div class="title-wrap">
        <h1 class="title">🗂️ All Meetings</h1>
      </div>
